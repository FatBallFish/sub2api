...
type GlobalPlanEligibilityChecker interface {
	HasApplicableRemaining(ctx context.Context, userID, groupID int64, now time.Time) (bool, error)
}
...
func (s *GlobalPlanService) HasApplicableRemaining(ctx context.Context, userID, groupID int64, now time.Time) (bool, error) {
	...
	snapshots, err := s.GetActiveList(ctx, userID, now)
	if err != nil {
		return false, err
	}
	for _, snapshot := range snapshots {
		if snapshot != nil && snapshot.QuotaRemainingUSD > 0 && globalPlanAppliesToGroup(snapshot, groupID) {
			return true, nil
		}
	}
	return false, nil
...
func globalPlanAppliesToGroup(snapshot *GlobalPlanSnapshot, groupID int64) bool {
	if snapshot == nil || groupID <= 0 {
		return false
	}
	mode := normalizePlanApplicableGroupMode(snapshot.ApplicableGroupMode)
	if mode == PlanApplicableGroupModeAll {
		return true
	}
	listed := false
	for _, id := range snapshot.ApplicableGroupIDs {
		if id == groupID {
			listed = true
			break
		}
	}
	if mode == PlanApplicableGroupModeWhitelist {
		return listed
	}
	return !listed
}

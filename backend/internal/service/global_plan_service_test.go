...
func TestGlobalPlanServiceHasApplicableRemainingKeepsDelistedPlanEntitlement(t *testing.T) {
	client := newGlobalPlanEntClient(t)
	ctx := context.Background()
	now := time.Date(2026, 7, 22, 12, 0, 0, 0, time.UTC)

	user := client.User.Create().
		SetEmail("delisted-global-plan@example.com").
		SetPasswordHash("hash").
		SetStatus(domain.StatusActive).
		SaveX(ctx)
	plan := client.SubscriptionPlan.Create().
		SetPlanScope(service.PlanScopeGlobal).
		SetName("Delisted Global").
		SetPrice(49).
		SetForSale(false).
		SetQuotaPeriod(service.GlobalPlanQuotaPeriodWeek).
		SetQuotaPerPeriodUsd(50).
		SetValidityDays(30).
	...
	sub := client.UserGlobalPlanSubscription.Create().
		...
		SetCurrentPeriodStart(now.Add(-24 * time.Hour)).
		SetCurrentPeriodEnd(now.Add(6 * 24 * time.Hour)).
		SetQuotaPeriod(service.GlobalPlanQuotaPeriodWeek).
		SetQuotaLimitUsd(50).
		SetQuotaUsedUsd(10).
		SetApplicableGroupMode(service.PlanApplicableGroupModeAll).
		SetPlanNameSnapshot("Delisted Global").
		SaveX(ctx)

	available, err := service.NewGlobalPlanService(client).HasApplicableRemaining(ctx, user.ID, 42, now)

	require.NoError(t, err)
	require.True(t, available)

	client.UserGlobalPlanSubscription.UpdateOneID(sub.ID).SetQuotaUsedUsd(50).SaveX(ctx)
	available, err = service.NewGlobalPlanService(client).HasApplicableRemaining(ctx, user.ID, 42, now)
	require.NoError(t, err)
	...
	cancel()
	available, err = service.NewGlobalPlanService(client).HasApplicableRemaining(cancelledCtx, user.ID, 42, now)
	require.Error(t, err)
	require.False(t, available)
}

...
func (s *paymentOrderUserRepoStub) CreateWithEmailAliasGuard(ctx context.Context, user *User) error {
...
func (s *paymentOrderUserRepoStub) Update(_ context.Context, user *User, _ UserUpdateFields) error {
	s.user = user
	return nil
}
...
func (s *paymentOrderUserRepoStub) AdjustBalance(_ context.Context, _ int64, delta float64) (BalanceChange, error) {
	if s.user == nil {
...
func (s *paymentOrderUserRepoStub) SetBalance(_ context.Context, _ int64, value float64) (BalanceChange, error) {
	if s.user == nil {
		return BalanceChange{}, ErrUserNotFound
	}
...
func (s *paymentOrderUserRepoStub) BatchUpdateLimits(context.Context, []int64, *int, *int) (int, error) {
	panic("unexpected BatchUpdateLimits call")
}
func (s *paymentOrderUserRepoStub) ExistsByEmail(context.Context, string) (bool, error) {
	return false, nil
}
func (s *paymentOrderUserRepoStub) ExistsByEmailAlias(ctx context.Context, email string) (bool, error) {
...
func TestBuildPaymentOrderProviderSnapshot_IncludesStripeWalletPreferences(t *testing.T) {
	t.Parallel()
	...
	require.Equal(t, "never", snapshot["stripe_apple_pay"])
	require.Equal(t, "auto", snapshot["stripe_google_pay"])
	require.NotContains(t, snapshot, "secretKey")

	preferences := PaymentOrderStripeWalletPreferences(&dbent.PaymentOrder{
		ProviderSnapshot: snapshot,
	})
	require.Equal(t, &StripeWalletDisplayPreferences{
		ApplePay:  "never",
		GooglePay: "auto",
	}, preferences)
}

func TestPaymentOrderStripeWalletPreferences_DefaultsLegacyStripeSnapshotToAuto(t *testing.T) {
	t.Parallel()
	...
	require.Nil(t, PaymentOrderStripeWalletPreferences(&dbent.PaymentOrder{
		ProviderSnapshot: map[string]any{
			"schema_version": 2,
			"provider_key":   payment.TypeAlipay,
		},
	}))
	require.Nil(t, PaymentOrderStripeWalletPreferences(nil))
	require.Nil(t, PaymentOrderStripeWalletPreferences(&dbent.PaymentOrder{
		ProviderSnapshot: map[string]any{
			"schema_version":    2,
			"provider_key":      payment.TypeStripe,
			"stripe_apple_pay":  "invalid",
			"stripe_google_pay": "auto",
		},
	}))
}

func TestBuildPaymentOrderProviderSnapshot_OmitsInvalidStripeWalletPreferences(t *testing.T) {
	t.Parallel()
	...
	require.NotContains(t, snapshot, "stripe_apple_pay")
	require.NotContains(t, snapshot, "stripe_google_pay")
}

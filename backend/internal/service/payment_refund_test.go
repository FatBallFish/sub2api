...
func TestGatewayRefundIDFitsJeepayMerchantRefundNoLimit(t *testing.T) {
	order := &dbent.PaymentOrder{
		ID:         7,
		OutTradeNo: "sub2_20260622VisJcWzm",
	}

	refundID := gatewayRefundID(&RefundPlan{
		OrderID:       order.ID,
		Order:         order,
		GatewayAmount: 0.68,
	})
	...
	require.LessOrEqual(t, len(refundID), 30)
	require.NotContains(t, refundID, ".")
	require.NotContains(t, refundID, " ")
	require.Contains(t, refundID, "22VisJcWzm")
}

func TestHandleGwFailReturnsErrorWhenRollbackSucceeded(t *testing.T) {
	ctx := context.Background()
	client := newPaymentConfigServiceTestClient(t)

	user, err := client.User.Create().
		SetEmail("refund-gateway-fail@example.com").
		SetPasswordHash("hash").
		SetUsername("refund-gateway-fail-user").
		Save(ctx)
	require.NoError(t, err)
	...
	order, err := client.PaymentOrder.Create().
		...
		SetPayAmount(68).
		SetFeeRate(0).
		SetRechargeCode("REFUND-GATEWAY-FAIL").
		SetOutTradeNo("sub2_refund_gateway_fail").
		SetPaymentType(payment.TypeJeepay).
		SetPaymentTradeNo("trade-refund-gateway-fail").
		SetOrderType(payment.OrderTypeBalance).
	...
	result, err := (&PaymentService{entClient: client}).handleGwFail(ctx, &RefundPlan{
		OrderID: order.ID,
		Order:   order,
	}, errors.New("jeepay refund error: 系统异常"))

	require.Nil(t, result)
	require.Error(t, err)
	require.Equal(t, "REFUND_FAILED", infraerrors.Reason(err))
	require.Contains(t, err.Error(), "jeepay refund error")
...
func TestPrepareRefundBalanceDeductUsesOrderRefundSnapshot(t *testing.T) {
	ctx := context.Background()
	client := newPaymentConfigServiceTestClient(t)

	user, err := client.User.Create().
		SetEmail("refund-balance-snapshot@example.com").
		SetPasswordHash("hash").
		SetUsername("refund-balance-snapshot-user").
		SetBalance(200).
		Save(ctx)
	require.NoError(t, err)
	...
	inst, err := client.PaymentProviderInstance.Create().
		SetProviderKey(payment.TypeAlipay).
		SetName("alipay-refund-snapshot-instance").
		SetConfig("{}").
		SetSupportedTypes("alipay").
		SetEnabled(true).
		SetRefundEnabled(true).
	...
	order, err := client.PaymentOrder.Create().
		...
		SetPayAmount(100).
		SetFeeRate(0).
		SetRechargeCode("REFUND-BALANCE-SNAPSHOT-ORDER").
		SetOutTradeNo("sub2_refund_balance_snapshot_order").
		SetPaymentType(payment.TypeAlipay).
		SetPaymentTradeNo("trade-refund-balance-snapshot").
		SetOrderType(payment.OrderTypeBalance).
		SetStatus(OrderStatusCompleted).
		SetExpiresAt(time.Now().Add(time.Hour)).
		SetPaidAt(time.Now()).
		...
		SetProviderInstanceID(instID).
		SetProviderKey(payment.TypeAlipay).
		SetRefundSnapshot(map[string]any{
			"schema_version": 1,
			"balance": map[string]any{
				"schema_version":       1,
				"paid_amount":          100,
				"credited_balance":     125,
				"credit_per_paid_unit": 1.25,
			},
		}).
		Save(ctx)
	require.NoError(t, err)
	...
	svc := &PaymentService{
		entClient: client,
		userRepo:  &mockUserRepo{getByIDUser: &User{ID: user.ID, Email: user.Email, Balance: 200}},
	}

	plan, result, err := svc.PrepareRefund(ctx, order.ID, 50, "partial balance refund", false, true)
	require.NoError(t, err)
	require.Nil(t, result)
	require.InDelta(t, 50, plan.RefundAmount, 1e-9)
	require.InDelta(t, 40, plan.GatewayAmount, 1e-9)
	require.InDelta(t, 50, plan.BalanceToDeduct, 1e-9)
	require.Contains(t, plan.Reason, "balance deducted")
}

func TestPrepareRefundBalanceDeductCapsGatewayRefundByAvailableBalance(t *testing.T) {
	ctx := context.Background()
	client := newPaymentConfigServiceTestClient(t)

	user, err := client.User.Create().
		SetEmail("refund-balance-cap@example.com").
		SetPasswordHash("hash").
		SetUsername("refund-balance-cap-user").
		SetBalance(30).
		Save(ctx)
	require.NoError(t, err)
	...
	inst, err := client.PaymentProviderInstance.Create().
		SetProviderKey(payment.TypeAlipay).
		SetName("alipay-refund-cap-instance").
		SetConfig("{}").
		SetSupportedTypes("alipay").
		SetEnabled(true).
		SetRefundEnabled(true).
	...
	order, err := client.PaymentOrder.Create().
		...
		SetPayAmount(100).
		SetFeeRate(0).
		SetRechargeCode("REFUND-BALANCE-CAP-ORDER").
		SetOutTradeNo("sub2_refund_balance_cap_order").
		SetPaymentType(payment.TypeAlipay).
		SetPaymentTradeNo("trade-refund-balance-cap").
		SetOrderType(payment.OrderTypeBalance).
		SetStatus(OrderStatusCompleted).
		SetExpiresAt(time.Now().Add(time.Hour)).
		SetPaidAt(time.Now()).
	...
	svc := &PaymentService{
		entClient: client,
		userRepo:  &mockUserRepo{getByIDUser: &User{ID: user.ID, Email: user.Email, Balance: 30}},
	}

	plan, result, err := svc.PrepareRefund(ctx, order.ID, 125, "balance cap refund", false, true)
	require.NoError(t, err)
	require.Nil(t, plan)
	require.NotNil(t, result)
	require.True(t, result.RequireForce)

	plan, result, err = svc.PrepareRefund(ctx, order.ID, 125, "balance cap refund", true, true)
	require.NoError(t, err)
	require.Nil(t, result)
	require.InDelta(t, 125, plan.RefundAmount, 1e-9)
	require.InDelta(t, 30, plan.BalanceToDeduct, 1e-9)
	require.InDelta(t, 24, plan.GatewayAmount, 1e-9)
}

func TestPrepareRefundGlobalPlanUsesLowerDurationOrQuotaRatio(t *testing.T) {
	ctx := context.Background()
	client := newPaymentConfigServiceTestClient(t)
	now := time.Now().UTC()

	user, err := client.User.Create().
		SetEmail("refund-global-plan@example.com").
		SetPasswordHash("hash").
		SetUsername("refund-global-plan-user").
		Save(ctx)
	require.NoError(t, err)
	inst, err := client.PaymentProviderInstance.Create().
		SetProviderKey(payment.TypeAlipay).
		SetName("alipay-refund-global-instance").
		SetConfig("{}").
		SetSupportedTypes("alipay").
		SetEnabled(true).
		SetRefundEnabled(true).
	...
	sub, err := client.UserGlobalPlanSubscription.Create().
		...
		SetCurrentPeriodStart(now.Add(-5 * 24 * time.Hour)).
		SetCurrentPeriodEnd(now.Add(2 * 24 * time.Hour)).
		SetQuotaPeriod(GlobalPlanQuotaPeriodWeek).
		SetQuotaLimitUsd(100).
		SetQuotaUsedUsd(80).
		SetTierRank(1).
		SetPlanNameSnapshot(planEnt.Name).
		Save(ctx)
	require.NoError(t, err)
	instID := strconv.FormatInt(inst.ID, 10)
	order, err := client.PaymentOrder.Create().
		...
		SetPayAmount(90).
		SetFeeRate(0).
		SetRechargeCode("REFUND-GLOBAL-PLAN-ORDER").
		SetOutTradeNo("sub2_refund_global_plan_order").
		SetPaymentType(payment.TypeAlipay).
		SetPaymentTradeNo("trade-refund-global-plan").
		SetOrderType(payment.OrderTypeGlobalPlan).
		SetStatus(OrderStatusCompleted).
		SetPlanID(planEnt.ID).
		SetPlanScope(PlanScopeGlobal).
		SetGlobalPlanSubscriptionID(sub.ID).
	...
	refundPlan, result, err := svc.PrepareRefund(ctx, order.ID, 0, "global refund", false, true)
	require.NoError(t, err)
	require.Nil(t, result)
	require.Equal(t, payment.DeductionTypeSubscription, refundPlan.DeductionType)
	require.Equal(t, sub.ID, refundPlan.SubscriptionID)
	require.Contains(t, refundPlan.Reason, "refund basis: remaining total quota")
	require.Less(t, refundPlan.RefundAmount, 90.0)
	require.Greater(t, refundPlan.RefundAmount, 0.0)
}

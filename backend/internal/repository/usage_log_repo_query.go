...
func scanUsageLog(scanner interface{ Scan(...any) error }) (*service.UsageLog, error) {
	...
	log := &service.UsageLog{
		...
		GlobalPlanCost:            globalPlanCost,
		BalanceCost:               balanceCost,
		GroupSubscriptionCost:     groupSubscriptionCost,
	...
	log.FundingSource = strings.TrimSpace(fundingSource)
	if globalPlanSubID.Valid {
		value := globalPlanSubID.Int64
		log.GlobalPlanSubscriptionID = &value
	}
	if durationMs.Valid {

...
const (
	...
	selectGlobalPlanForBillingSQL = `(?s)SELECT id, GREATEST\(quota_limit_usd - quota_used_usd, 0\)\s+FROM user_global_plan_subscriptions.*FOR UPDATE`
...
func TestApplyUsageBillingEffects_FlagsBalanceOverdraft(t *testing.T) {
	...
	tx, err := db.BeginTx(ctx, nil)
	require.NoError(t, err)
	mock.ExpectQuery(selectGlobalPlanForBillingSQL).
		WithArgs(int64(42), service.GlobalPlanStatusActive, int64(0)).
		WillReturnError(sql.ErrNoRows)
	mock.ExpectQuery(conditionalBalanceDeductSQL).
		WithArgs(10.0, int64(42)).
		WillReturnError(sql.ErrNoRows)

...
type UsageLogQuery struct {
	...
	withGlobalPlanSubscription *UserGlobalPlanSubscriptionQuery
...
// QueryGlobalPlanSubscription chains the current query on the "global_plan_subscription" edge.
func (_q *UsageLogQuery) QueryGlobalPlanSubscription() *UserGlobalPlanSubscriptionQuery {
...
func (_q *UsageLogQuery) Clone() *UsageLogQuery {
	...
	return &UsageLogQuery{
		...
		withGlobalPlanSubscription: _q.withGlobalPlanSubscription.Clone(),
...
// WithGlobalPlanSubscription tells the query-builder to eager-load the nodes that are connected to
// the "global_plan_subscription" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *UsageLogQuery) WithGlobalPlanSubscription(opts ...func(*UserGlobalPlanSubscriptionQuery)) *UsageLogQuery {
	query := (&UserGlobalPlanSubscriptionClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withGlobalPlanSubscription = query
	return _q
}
...
func (_q *UsageLogQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*UsageLog, error) {
	var (
		...
		loadedTypes = [6]bool{
			...
			_q.withGlobalPlanSubscription != nil,
		}
	)
	...
	if query := _q.withGlobalPlanSubscription; query != nil {
		if err := _q.loadGlobalPlanSubscription(ctx, query, nodes, nil,
			func(n *UsageLog, e *UserGlobalPlanSubscription) { n.Edges.GlobalPlanSubscription = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *UsageLogQuery) loadGlobalPlanSubscription(ctx context.Context, query *UserGlobalPlanSubscriptionQuery, nodes []*UsageLog, init func(*UsageLog), assign func(*UsageLog, *UserGlobalPlanSubscription)) error {
	ids := make([]int64, 0, len(nodes))
	nodeids := make(map[int64][]*UsageLog)
	for i := range nodes {
		if nodes[i].GlobalPlanSubscriptionID == nil {
			continue
		}
		fk := *nodes[i].GlobalPlanSubscriptionID
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	query.Where(userglobalplansubscription.IDIn(ids...))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	}
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "global_plan_subscription_id" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (_q *UsageLogQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := _q.ctx.Fields; len(fields) > 0 {
		...
		if _q.withGlobalPlanSubscription != nil {
			_spec.Node.AddColumnOnce(usagelog.FieldGlobalPlanSubscriptionID)
		}
	}

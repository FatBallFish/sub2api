...
func (_q *PaymentOrderQuery) QueryGlobalPlanSubscriptions() *UserGlobalPlanSubscriptionQuery {
...
func (_q *PaymentOrderQuery) WithGlobalPlanSubscriptions(opts ...func(*UserGlobalPlanSubscriptionQuery)) *PaymentOrderQuery {
	query := (&UserGlobalPlanSubscriptionClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *PaymentOrderQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*PaymentOrder, error) {
	...
	if query := _q.withGlobalPlanSubscriptions; query != nil {
		if err := _q.loadGlobalPlanSubscriptions(ctx, query, nodes,
			func(n *PaymentOrder) { n.Edges.GlobalPlanSubscriptions = []*UserGlobalPlanSubscription{} },
			func(n *PaymentOrder, e *UserGlobalPlanSubscription) {
...
func (_q *PaymentOrderQuery) loadGlobalPlanSubscriptions(ctx context.Context, query *UserGlobalPlanSubscriptionQuery, nodes []*PaymentOrder, init func(*PaymentOrder), assign func(*PaymentOrder, *UserGlobalPlanSubscription)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(userglobalplansubscription.FieldSourceOrderID)
	}
	query.Where(predicate.UserGlobalPlanSubscription(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(paymentorder.GlobalPlanSubscriptionsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err

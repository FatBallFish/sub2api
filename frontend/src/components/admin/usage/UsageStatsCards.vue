  ...
  <div class="space-y-4">
    ...
    <div class="grid grid-cols-1 gap-4 md:grid-cols-3">
      <div class="card p-4 flex items-center gap-3">
        ...
        <div class="min-w-0 flex-1">
          <p class="text-xs font-medium text-gray-500">{{ t('usage.remainingBalanceCredits') }}</p>
          <p class="text-xl font-bold text-cyan-600">${{ formatCredits(remainingBalanceCredits) }}</p>
          <p class="text-xs text-gray-400">{{ t('usage.currentInventory') }}</p>
        </div>
      </div>
      <div class="card p-4 flex items-center gap-3">
        ...
        <div class="min-w-0 flex-1">
          <p class="text-xs font-medium text-gray-500">{{ t('usage.remainingSubscriptionCredits') }}</p>
          <p class="text-xl font-bold text-indigo-600">${{ formatCredits(remainingSubscriptionCredits) }}</p>
          <p class="text-xs text-gray-400">{{ t('usage.currentWindowOnly') }}</p>
        </div>
      </div>
      <div class="card p-4 flex items-center gap-3">
        <div class="rounded-lg bg-emerald-100 p-2 dark:bg-emerald-900/30 text-emerald-600">
          <Icon name="dollar" size="md" />
        </div>
        <div class="min-w-0 flex-1">
          <p class="text-xs font-medium text-gray-500">{{ t('usage.remainingTotalCredits') }}</p>
          <p class="text-xl font-bold text-emerald-600">${{ formatCredits(remainingTotalCredits) }}</p>
          <p class="text-xs text-gray-400">{{ t('usage.balancePlusSubscription') }}</p>
        </div>
      </div>
    </div>
  </div>
...
const remainingBalanceCredits = computed(() => {
  const stats = props.stats as (AdminUsageStatsResponse & { remaining_balance_credits?: number }) | null
  return stats?.remaining_balance_credits || 0
})
const remainingSubscriptionCredits = computed(() => {
  const stats = props.stats as (AdminUsageStatsResponse & { remaining_subscription_credits?: number }) | null
  return stats?.remaining_subscription_credits || 0
})
const remainingTotalCredits = computed(() => remainingBalanceCredits.value + remainingSubscriptionCredits.value)

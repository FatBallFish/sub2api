...
const { list, groups, proxies } = vi.hoisted(() => ({
  list: vi.fn(),
  groups: vi.fn(),
  proxies: vi.fn()
}))
...
vi.mock('@/api/admin', () => ({
  adminAPI: {
    accounts: { list },
    groups: { getAllIncludingInactive: groups },
    proxies: { getAll: proxies }
  }
}))
...
describe('passive plugin resource directory', () => {
  ...
  it('loads every page while stopped, restricts accounts and strips secrets', async () => {
    ...
    proxies.mockResolvedValue([
      ...
      { id: 7, status: 'active', expires_at: '2000-01-01T00:00:00Z' }
    ])

    expect(await loadPluginResources(plugin)).toEqual({
      accounts: [
        { id: 1, name: 'One', group_ids: [3, 4] },
        { id: 2, name: 'Two', group_ids: [] }
      ],
  ...
  it('rejects a plugin without matching capability before any API reads', async () => {
    await expect(loadPluginResources({
      ...plugin,
      manifest: { ...plugin.manifest, capabilities: [] }
    })).rejects.toThrow('权限')
  ...
  it('does not deliver a partial directory when a later page fails', async () => {
    list.mockResolvedValueOnce({ pages: 2, items: [
      { id: 1, name: 'One', platform: 'openai', type: 'oauth' }
